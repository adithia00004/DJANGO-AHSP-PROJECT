...
| U19 | 🔴 Tinggi | **Stored XSS antar-user di Template Library**: `renderTemplateList` meng-interpolasi `t.name` (input user; template publik tampil lintas-user) ke `innerHTML` TANPA escape — ditemukan saat implementasi keputusan B2.5 | **FIXED**: `escapeHtml()` + koersi Number utk stats. Sekaligus keputusan produk (opsi A): template **private-by-default** (migration 0045), publik = kurasi admin (`PekerjaanTemplateAdmin`, `list_editable is_public`); API create memaksa privat utk non-staff; badge "Publik"/"Milik Saya" di UI. Test 39 passed |
