...
| R6/R4 | Quick-win + kit UAT (2026-06-10 13:05 WITA) | F5 (entrypoint `pg_isready -U $POSTGRES_USER`), F6 (git dihapus dari runtime image — 0 pemakaian runtime terverifikasi), F7 (`.env.pgbouncer` → `.example`); verifikasi test opaque M5+R1 ternyata sudah ada (memori lama usang); `UAT_SCRIPT_R4_20260610.md` dibuat (8 seksi, 4 akun role); `npm run build` segar |
