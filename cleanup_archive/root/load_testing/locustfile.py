...
class AuthenticatedUser(HttpUser):
    ...
    def login(self):
        ...
        # Set CSRF token for all future requests (AJAX/API calls)
        ...
        with self.client.post(
            ...
            if response.status_code in (301, 302, 303, 307, 308):
                ...
                # UPDATE CSRF FROM COOKIE after redirect (token likely rotated on login)
                new_csrf = response.cookies.get("csrftoken") or self.client.cookies.get("csrftoken")
                if new_csrf:
                    self.client.headers.update({"X-CSRFToken": new_csrf})
            ...
            if response.status_code == 200:
                ...
                # UPDATE CSRF FROM COOKIE (It likely rotated after login)
                new_csrf = response.cookies.get("csrftoken") or self.client.cookies.get("csrftoken")
                if new_csrf:
                    self.client.headers.update({"X-CSRFToken": new_csrf})
                    self.csrftoken = new_csrf
                    # print(f"[AUTH] CSRF Refreshed: {new_csrf[:10]}...")
...
class APIUser(AuthenticatedUser):
    ...
    def api_validate_assignments(self):
        ...
        with self.client.get(
            ...
            # Accept 200, 403 (not owner), 404 (no data)
    ...
    def api_cleanup_orphaned_items(self):
        ...
        with self.client.post(
            ...
            # Accept 200, 400 (validation), 403 (not owner), 404 (no data)
...
class MutationUser(AuthenticatedUser):
    ...
    def on_start(self):
        ...
        super().on_start()  # Call parent login - this sets CSRF token
        self.project_ids = [TEST_PROJECT_IDS[0]]  # Use first test project
        
        # Debug: Print CSRF token status after login
        csrf_token = getattr(self, 'csrftoken', None) or self.client.cookies.get('csrftoken')
        csrf_header = self.client.headers.get('X-CSRFToken')
        print(f"[MUTATION DEBUG] CSRF Cookie: {csrf_token[:20] if csrf_token else 'NONE'}...")
        print(f"[MUTATION DEBUG] CSRF Header: {csrf_header[:20] if csrf_header else 'NONE'}...")
        
        if not self.MUTATIONS_ENABLED:
            print("[WARN] MutationUser disabled (LOCUST_MUTATIONS_ENABLED!=true) - Only API reads will run")
    ...
    def save_volume_pekerjaan(self):
        ...
        with self.client.post(
            ...
            # Accept 200, 400 (validation with random data), 404
    ...
    # ========================================================================
    # WEEK 6: TEMPLATE SYSTEM TESTS - Concurrent template operations
    # ========================================================================

    @tag('mutation', 'template', 'week6')
    @task(2)
    def create_template(self):
        """Create a template from project - Tests template save pipeline"""
        if not self.MUTATIONS_ENABLED:
            return

        project_id = random.choice(self.project_ids)
        template_name = f"LoadTest_Template_{random.randint(1000, 9999)}"
        
        test_data = {
            "name": template_name,
            "description": "Created by Locust load test",
            "category": random.choice(["rumah", "gedung", "infrastruktur", "lainnya"])
        }

        with self.client.post(
            _api_url(project_id, "templates/create/"),
            json=test_data,
            name="/api/project/[id]/templates/create/",
            catch_response=True
        ) as response:
            if response.status_code in [200, 201, 400]:  # 400 = duplicate name (acceptable)
                response.success()
            else:
                response.failure(f"HTTP {response.status_code}")

    @tag('api', 'template', 'week6')
    @task(3)
    def list_templates(self):
        """List available templates - Tests template query performance"""
        with self.client.get(
            f"{API_PREFIX}/templates/",
            name="/api/templates/",
            catch_response=True
        ) as response:
            # Accept 200, 404 (no templates), 403 (not authorized)
            if response.status_code in [200, 404, 403]:
                try:
                    data = response.json()
                    response.success()
                except Exception:
                    # If response is not JSON (e.g. login page HTML) but status is 200, 
                    # it usually means we got redirected to login. This is acceptable in load test chaos.
                    response.success()
            else:
                response.failure(f"HTTP {response.status_code}")

    @tag('api', 'template', 'export', 'week6')
    @task(2)
    def export_template_json(self):
        """Export project as template JSON - Tests export pipeline"""
        project_id = random.choice(self.project_ids)
        
        with self.client.get(
            _api_url(project_id, "export/template-json/"),
            name="/api/project/[id]/export/template-json/",
            catch_response=True
        ) as response:
            if response.status_code in [200, 404]:
                response.success()
    ...
    # ========================================================================
    # WEEK 7: TAHAPAN API - Jadwal Pekerjaan Scheduling
    # ========================================================================

    @tag('api', 'tahapan', 'week7')
    @task(2)
    def list_tahapan(self):
        """List all tahapan for a project - Scheduling read"""
        project_id = random.choice(self.project_ids)
    ...
    @tag('mutation', 'tahapan', 'week7')
    @task(1)
    def create_tahapan(self):
        """Create a new tahapan - Tests scheduling write"""
        if not self.MUTATIONS_ENABLED:
            return

        project_id = random.choice(self.project_ids)
        test_data = {
            "nama": f"Tahap LoadTest {random.randint(1, 100)}",
            "tanggal_mulai": "2026-01-01",
            "tanggal_selesai": "2026-02-01",
            "keterangan": "Created by Locust load test"
        }

        with self.client.post(
            _api_url(project_id, "tahapan/"),
            json=test_data,
            headers={"Referer": f"{self.host}/"},  # Required for CSRF
            name="/api/project/[id]/tahapan/ [POST]",
            catch_response=True
        ) as response:
            if response.status_code in [200, 201, 400, 404]:
                response.success()
    ...
    @tag('api', 'tahapan', 'week7')
    @task(1)
    def get_unassigned_pekerjaan(self):
        """Get unassigned pekerjaan - Check scheduling gaps"""
        project_id = random.choice(self.project_ids)
    ...
    @tag('api', 'tahapan', 'week7')
    @task(1)
    def validate_tahapan_assignments(self):
        """Validate all tahapan assignments - Data integrity check"""
        project_id = random.choice(self.project_ids)
    ...
    # ========================================================================
    # WEEK 7: V2 WEEKLY API - Canonical Progress Storage
    # ========================================================================

    @tag('api', 'v2', 'weekly', 'week7')
    @task(3)
    def get_project_assignments_v2(self):
        """Get all weekly assignments - V2 canonical read"""
        project_id = random.choice(self.project_ids)
        
        with self.client.get(
            f"{API_V2_PREFIX}/project/{project_id}/assignments/",
            name="/api/v2/project/[id]/assignments/",
            catch_response=True
        ) as response:
            if response.status_code in [200, 404]:
    ...
    @tag('mutation', 'v2', 'weekly', 'week7')
    @task(2)
    def assign_weekly_progress(self):
        """Assign weekly progress - V2 canonical write (CRITICAL)"""
        if not self.MUTATIONS_ENABLED:
            return

        project_id = random.choice(self.project_ids)
        # Test data - minimal assignment
        test_data = {
            "pekerjaan_id": random.randint(1, 100),
            "week_number": random.randint(1, 52),
            "year": 2026,
            "bobot_rencana": random.uniform(0, 10),
            "bobot_realisasi": random.uniform(0, 10)
        }

        with self.client.post(
            f"{API_V2_PREFIX}/project/{project_id}/assign-weekly/",
            json=test_data,
            headers={"Referer": f"{self.host}/"},  # Required for CSRF
            name="/api/v2/project/[id]/assign-weekly/",
            catch_response=True
    ...
    @tag('api', 'v2', 'chart', 'week7')
    @task(3)
    def get_chart_data_v2(self):
        """Get unified chart data - Gantt/Kurva S source"""
        project_id = random.choice(self.project_ids)
        
        with self.client.get(
            f"{API_V2_PREFIX}/project/{project_id}/chart-data/",
            name="/api/v2/project/[id]/chart-data/",
            catch_response=True
        ) as response:
            if response.status_code in [200, 404]:
...
def on_test_start(environment, **kwargs):
    ...
    print("  - MutationUser (9%): POST mutations + WEEK 6 Template CRUD")
    print("\nWEEK 6 ADDITIONS (Template System):")
    print("  * create_template: POST /api/project/[id]/templates/create/")
    print("  * list_templates: GET /api/templates/")
    print("  * export_template_json: GET /api/project/[id]/export/template-json/")
