...
- Panel kiri AG Grid memuat kolom Kode/Pekerjaan/Volume/Satuan (pinned) dan theme otomatis mengikuti `data-bs-theme`, sehingga UX konsisten di mode terang maupun gelap.
- **Phase 2 assignments v2** aktif: DataLoader kini membaca endpoint `/api/v2/project/<id>/assignments/` secara penuh tanpa fallback legacy, sehingga weekly canonical menjadi satu-satunya sumber data untuk semua mode grid.
