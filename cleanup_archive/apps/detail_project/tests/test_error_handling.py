...
def test_get_error_response_helper():
...
def test_get_error_response_with_error_id():
...
def test_classify_integrity_error_unique():
...
def test_classify_integrity_error_foreign_key():
...
def test_classify_operational_error_deadlock():
...
@pytest.fixture
def validation_user(db):
    return User.objects.create_user(username='testuser', password='12345')


@pytest.fixture
def validation_project(validation_user):
    return Project.objects.create(
        owner=validation_user,
        nama="Test Project",
        sumber_dana="APBN",
...
def test_empty_name_raises_error(validation_project, validation_user):
    """Test empty project name raises ValidationError."""
    service = DeepCopyService(validation_project)

    with pytest.raises(DeepCopyValidationError) as exc_info:
        service.copy(validation_user, "")

    assert exc_info.value.code == 1001
    assert "kosong" in exc_info.value.user_message.lower()


def test_name_too_long_raises_error(validation_project, validation_user):
    """Test project name > 200 chars raises ValidationError."""
    service = DeepCopyService(validation_project)
    long_name = "A" * 201

    with pytest.raises(DeepCopyValidationError) as exc_info:
        service.copy(validation_user, long_name)

    assert exc_info.value.code == 1004
    assert exc_info.value.details['length'] == 201


def test_xss_in_name_raises_error(validation_project, validation_user):
    """Test XSS characters in name raises ValidationError."""
    service = DeepCopyService(validation_project)

    with pytest.raises(DeepCopyValidationError) as exc_info:
        service.copy(validation_user, "Project <script>alert(1)</script>")

    assert exc_info.value.code == 1007
    assert "<>" in exc_info.value.details['invalid_chars']


def test_invalid_date_range_raises_error(validation_project, validation_user):
    """Test invalid date range raises ValidationError."""
    service = DeepCopyService(validation_project)

    with pytest.raises(DeepCopyValidationError) as exc_info:
        service.copy(validation_user, "Valid Name", new_tanggal_mulai=date(2150, 1, 1))

    assert exc_info.value.code == 1003


def test_duplicate_name_raises_error(validation_project, validation_user):
    ...
    with pytest.raises(DeepCopyBusinessError) as exc_info:
        service.copy(validation_user, "Duplicate Name")
...
@pytest.fixture
def project_with_orphans(skip_user):
...
def test_skip_tracking_works(skip_user):
    ...
    new_project = service.copy(skip_user, "Copy Project Skip Tracking")
...
def test_warnings_collection(skip_user):
    ...
    service = DeepCopyService(project)
    new_project = service.copy(skip_user, "Copy Project Warnings")
...
def test_get_skipped_items_returns_copy(validation_project, validation_user):
    """Test that get_skipped_items() returns a copy."""
    service = DeepCopyService(validation_project)
    new_project = service.copy(validation_user, "Copy Project")
...
def test_empty_name_returns_400(client_logged, user, db):
    """Test empty name returns 400 with error code."""
    # Create api_project with the same user as client_logged
    ...
    response = client_logged.post(
        f'/detail_project/api/project/{api_project.pk}/deep-copy/',
...
def test_duplicate_name_returns_400(client_logged, user, db):
    """Test duplicate name returns 400 with business error."""
    # Create api_project with the same user as client_logged
    api_project = Project.objects.create(
        owner=user,
        nama="API Test Project Dup",
        sumber_dana="APBN",
        lokasi_project="Jakarta",
        nama_client="Client",
        anggaran_owner=Decimal("1000000"),
    ...
    # Create existing project with same owner
    Project.objects.create(
        owner=user,
...
def test_success_returns_warnings(client_logged, user, db):
    """Test successful copy returns warnings if any."""
    # Create api_project with the same user as client_logged
    api_project = Project.objects.create(
        owner=user,
        nama="API Test Project Success",
        sumber_dana="APBN",
        lokasi_project="Jakarta",
        nama_client="Client",
        anggaran_owner=Decimal("1000000"),
...
def test_invalid_json_returns_400(client_logged, user, db):
    """Test invalid JSON returns 400."""
    # Create api_project with the same user as client_logged
    api_project = Project.objects.create(
        owner=user,
        nama="API Test Project Invalid",
        sumber_dana="APBN",
        lokasi_project="Jakarta",
        nama_client="Client",
        anggaran_owner=Decimal("1000000"),
...
@pytest.fixture
def full_project(integration_user):
...
def test_successful_copy_with_all_features(full_project, integration_user):
    """Test successful copy with all error handling features active."""
    service = DeepCopyService(full_project)

    new_project = service.copy(integration_user, "Complete Copy")
...
def test_error_logging_works(full_project, integration_user, caplog):
    """Test that errors are logged properly."""
    import logging
    caplog.set_level(logging.INFO)

    service = DeepCopyService(full_project)
    new_project = service.copy(integration_user, "Logged Copy")
