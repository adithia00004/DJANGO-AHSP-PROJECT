...
@pytest.mark.django_db
class TestAutoGeneratedUnitKode:
    """Ensure kode otomatis 'Unit-XXXX' terisi saat user mengosongkan kode."""

    def test_missing_kode_auto_populated(self, client_logged, project, setup_template_ahsp_test):
        pekerjaan = setup_template_ahsp_test['pekerjaan']
        url = reverse('detail_project:api_save_detail_ahsp_for_pekerjaan',
                      kwargs={'project_id': project.id, 'pekerjaan_id': pekerjaan.id})
        ...
        codes = sorted(
            DetailAHSPProject.objects.filter(project=project, pekerjaan=pekerjaan)
            .order_by('kategori')
            .values_list('kode', flat=True)
        )
        assert codes == ["Unit-0001", "Unit-0002"]

    def test_sequence_persists_between_saves(self, client_logged, project, setup_template_ahsp_test):
        pekerjaan = setup_template_ahsp_test['pekerjaan']
        url = reverse('detail_project:api_save_detail_ahsp_for_pekerjaan',
                      kwargs={'project_id': project.id, 'pekerjaan_id': pekerjaan.id})
        ...
        resp_first = client_logged.post(
            url,
            data=json.dumps(payload_first),
            content_type='application/json',
        )
        assert resp_first.status_code == 200, resp_first.content
        ...
        resp_second = client_logged.post(
            url,
            data=json.dumps(payload_second),
            content_type='application/json',
        )
        assert resp_second.status_code == 200, resp_second.content

        final_codes = list(
            DetailAHSPProject.objects.filter(project=project, pekerjaan=pekerjaan)
            .values_list('kode', flat=True)
        )
        assert final_codes == ["Unit-0003"]

        tracker = ProjectChangeStatus.objects.get(project=project)
        assert tracker.unit_code_sequence == 3
