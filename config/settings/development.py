...
# ---------------------------------------------------------------------------
# SQL trace logging (development only, opt-in via SQL_TRACE=1)
# ---------------------------------------------------------------------------

SQL_TRACE_ENABLED = os.getenv("SQL_TRACE", "False").lower() in ("true", "1", "yes")
if SQL_TRACE_ENABLED:
    sql_trace_log_path = BASE_DIR / "logs" / "sql_trace.log"
    sql_trace_log_path.parent.mkdir(parents=True, exist_ok=True)

    LOGGING.setdefault("handlers", {})
    LOGGING.setdefault("loggers", {})

    LOGGING["handlers"]["sql_trace_file"] = {
        "class": "logging.FileHandler",
        "filename": str(sql_trace_log_path),
        "formatter": "standard",
    }
    LOGGING["loggers"]["django.db.backends"] = {
        "handlers": ["sql_trace_file"],
        "level": "DEBUG",
        "propagate": False,
    }
    LOGGING["loggers"]["sql_trace"] = {
        "handlers": ["sql_trace_file", "console"],
        "level": "INFO",
        "propagate": False,
    }
    if "config.middleware.sql_trace.SQLTraceMiddleware" not in MIDDLEWARE:
        MIDDLEWARE.insert(0, "config.middleware.sql_trace.SQLTraceMiddleware")
