...
class JadwalPekerjaanExportAdapter:
    ...
    def __init__(
        ...
        self._bobot_cache: Tuple[Dict[int, Decimal], Decimal, Dict[int, Decimal]] | None = None
    ...
    def _build_progress_map(self) -> Tuple[Dict[Tuple[int, int], Decimal], Dict[str, Any]]:
        ...
        weekly_qs = (
            PekerjaanProgressWeekly.objects.filter(project=self.project)
            .values("pekerjaan_id", "week_number")
            .annotate(
                planned_sum=Sum("planned_proportion"),
                week_start=Min("week_start_date"),
                week_end=Max("week_end_date"),
            )
        )
        ...
        for row in weekly_qs:
            ...
            planned_proportion = row["planned_sum"]
            week_start_date = row["week_start"]
            week_end_date = row["week_end"]
            key = (pekerjaan_id, week_number)
            try:
                progress_map[key] = Decimal(planned_proportion or 0)
            except InvalidOperation:
                progress_map[key] = Decimal("0")

            if week_number and week_number > max_week_number:
                max_week_number = week_number
            if week_start_date and (earliest_start is None or week_start_date < earliest_start):
                earliest_start = week_start_date
            if week_end_date and (latest_end is None or week_end_date > latest_end):
                latest_end = week_end_date
    ...
    def _build_actual_progress_map(self) -> Dict[Tuple[int, int], Decimal]:
        ...
        weekly_qs = (
            PekerjaanProgressWeekly.objects.filter(project=self.project)
            .values("pekerjaan_id", "week_number")
            .annotate(actual_sum=Sum("actual_proportion"))
        )

        for row in weekly_qs:
            pekerjaan_id = row["pekerjaan_id"]
            week_number = row["week_number"]
            actual_proportion = row["actual_sum"]
            key = (pekerjaan_id, week_number)
            try:
                actual_map[key] = Decimal(actual_proportion or 0)
            except InvalidOperation:
    ...
    def _calculate_kurva_s_data(
        ...
        valid_weeks = {col.get("week_number") for col in weekly_columns if col.get("week_number")}

        # DB aggregation: reduce Python looping by aggregating per week in the database.
        planned_by_week, actual_by_week = self._build_weighted_weekly_totals(
            bobot_map, valid_weeks
        )
        ...
        for col in weekly_columns:
            ...
            week_planned_weighted = planned_by_week.get(week_number, Decimal("0"))
            week_actual_weighted = actual_by_week.get(week_number, Decimal("0"))
    ...
    def _calculate_project_summary(
        ...
        """
        total_weeks = len(weekly_columns)
        pekerjaan_count = sum(1 for row in base_rows if row.get("type") == "pekerjaan")

        _, _, bobot_map = self._get_bobot_maps(base_rows)
        valid_weeks = {col.get("week_number") for col in weekly_columns if col.get("week_number")}
        # DB aggregation: compute weighted totals from aggregated weekly data.
        weighted_planned, weighted_actual = self._build_weighted_totals(
            bobot_map, valid_weeks
        )
    ...
    def _build_weighted_weekly_totals(
        self,
        bobot_map: Dict[int, Decimal],
        valid_weeks: Set[int],
    ) -> Tuple[Dict[int, Decimal], Dict[int, Decimal]]:
        planned_by_week: Dict[int, Decimal] = {}
        actual_by_week: Dict[int, Decimal] = {}

        weekly_qs = PekerjaanProgressWeekly.objects.filter(project=self.project)
        if valid_weeks:
            weekly_qs = weekly_qs.filter(week_number__in=valid_weeks)
        weekly_qs = weekly_qs.values("week_number", "pekerjaan_id").annotate(
            planned_sum=Sum("planned_proportion"),
            actual_sum=Sum("actual_proportion"),
        )

        for row in weekly_qs:
            bobot = bobot_map.get(row["pekerjaan_id"])
            if not bobot:
                continue
            week_number = row["week_number"]
            planned_val = Decimal(row["planned_sum"] or 0)
            actual_val = Decimal(row["actual_sum"] or 0)

            planned_by_week[week_number] = planned_by_week.get(week_number, Decimal("0")) + (
                planned_val * bobot
            )
            actual_by_week[week_number] = actual_by_week.get(week_number, Decimal("0")) + (
                actual_val * bobot
            )

        return planned_by_week, actual_by_week

    def _build_weighted_totals(
        self,
        bobot_map: Dict[int, Decimal],
        valid_weeks: Set[int],
        ...
        totals_qs = PekerjaanProgressWeekly.objects.filter(project=self.project)
        if valid_weeks:
            totals_qs = totals_qs.filter(week_number__in=valid_weeks)
        totals_qs = totals_qs.values("pekerjaan_id").annotate(
            planned_sum=Sum("planned_proportion"),
            actual_sum=Sum("actual_proportion"),
        )
        ...
        for row in totals_qs:
            ...
            if not bobot:
                continue
            weighted_planned += Decimal(row["planned_sum"] or 0) * bobot
            weighted_actual += Decimal(row["actual_sum"] or 0) * bobot
    ...
    def _get_bobot_maps(
        self, base_rows: Sequence[Dict[str, Any]]
    ) -> Tuple[Dict[int, Decimal], Decimal, Dict[int, Decimal]]:
        """
        Build and cache harga + bobot maps for progress weighting.
        """
        if self._bobot_cache is not None:
            return self._bobot_cache

        harga_map: Dict[int, Decimal] = {}
        total_harga = Decimal("0")
        for row in base_rows:
            if row.get("type") != "pekerjaan":
                continue
        ...
        bobot_map = {pek_id: harga / total_harga for pek_id, harga in harga_map.items()}
        self._bobot_cache = (harga_map, total_harga, bobot_map)
        return self._bobot_cache
