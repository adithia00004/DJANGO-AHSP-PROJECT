...
- **Template aktif:** `kelola_tahapan_grid_modern.html` (Vite). TanStack grid + modul jadwal modern menjadi default; flag `ENABLE_AG_GRID` telah dihapus sehingga kontainer legacy tidak lagi dirender. Panel kiri tetap terpin dengan status bar aktif dan dual horizontal scrollbar untuk memastikan sinkron scroll dengan Kurva S & Gantt.
...
- **Mode biaya realisasi (Phase 5.3):** Saat toggle progres berada di **Realisasi**, toolbar menampilkan opsi tampilan **Cost** dan kini otomatis aktif ketika halaman selesai memuat sehingga Kurva S uPlot langsung menampilkan data biaya. Dataset mingguannya menyertakan PV/AC kumulatif, delta persen, serta delta rupiah (planned vs actual) sehingga tooltip menampilkan kombinasi % dan Rupiah sekaligus. Legend hanya menampilkan **Rencana (PV)** vs **Realisasi (AC)** dan Week 0 (0%) tetap disisipkan sebagai baseline.
...
- **TanStack Grid:** modul utama `modules/grid/tanstack-grid-manager.js` menangani virtual scroll + inline editor. Legacy AG Grid sudah dihapus dari bundle; semua toolbar/event bekerja melalui StateManager sehingga grid dan Kurva S selalu sinkron.
...
- **Kurva S modern**: modul `modules/kurva-s/uplot-chart.js` terintegrasi penuh dengan state manager. Pada mode **Realisasi**, Jadwal Kegiatan App selalu mengaktifkan tampilan **Cost** dan mem-fetch `/detail_project/api/v2/project/<id>/kurva-s-harga/`. Data PV/AC dinormalisasi (Week 0 = 0%) dan legend memakai label **Rencana (PV)** vs **Realisasi (AC)**, dilengkapi tooltip delta persen + Rupiah sehingga kurva langsung mencerminkan input planned/actual. Mode progres (volume/persentase) tetap tersedia ketika user berpindah tab.
