...
- **Mode biaya realisasi (Phase 5.3):** Saat toggle progres berada di **Realisasi**, toolbar menampilkan opsi tampilan **Cost** dan kini otomatis aktif ketika halaman selesai memuat sehingga Kurva S langsung menampilkan data biaya. Mode ini mengganti sel AG Grid menjadi input rupiah dan SaveHandler menyisipkan actual_cost bersamaan dengan actual_proportion, sehingga backend merekam biaya aktual mingguan dan mengembalikannya lewat kunci saved_assignments. Grafik Kurva S akan selalu menyisipkan Week 0 (0%) agar baseline proyek terlihat, dengan legend yang hanya menampilkan **Rencana (PV)** vs **Realisasi (AC)**.
